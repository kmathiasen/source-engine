...
public LL_OnPluginStart()
{
    ...
    {
        bLLDisabled = true;
        LogError("No location data file \"./data/letters.txt\"");
    }
    ...
    hMaxPerTime = CreateConVar("hg_premium_ll_max_letters_per_time", "50",
                                "The maximum amount of letters an admin can use per time interval");

    hMaxInterval = CreateConVar("hg_premium__ll_max_letters_interval", "600",
                                "The time interval for the max letters an admin can use");

    hDrawDelay = CreateConVar("hg_premium_ll_draw_delay", "0.05",
                              "How long to delay drawing each consecutive letter");

    hCanUseWhileDead = CreateConVar("hg_premium_ll_can_use_while_dead", "1",
    ...
    RegServerCmd("hg_items_ll", Command_DrawText);
}

public LL_OnConfigsExecuted()
{
    iMaxPerTime = GetConVarInt(hMaxPerTime);
    iMaxInterval = GetConVarInt(hMaxInterval);
    fDrawDelay = GetConVarFloat(hDrawDelay);
    bCanUseWhileDead = GetConVarBool(hCanUseWhileDead);
}

public LL_OnConVarChanged(Handle:CVar, const String:sOld[], const String:sNew[])
{
    if (CVar == hMaxPerTime)
    {
        iMaxPerTime = GetConVarInt(hMaxPerTime);
...
public Action:Command_DrawText(args)
{
    if (args < 1 || bLLDisabled)
    ...
    if (args < 2)
    {
        PrintToChat(client, "%s Invalid syntax -- \x03!ll <text>", MSG_PREFIX);
        return Plugin_Handled;
    }

    Format(text, sizeof(text), "%s", sArgString[StrContains(sArgString, sUserid) + strlen(sUserid)]);

    if (!bCanUseWhileDead &&
        (!IsPlayerAlive(client) || GetClientTeam(client) <= 1))
    {
        PrintToChat(client, "%s You must be alive to use this command", MSG_PREFIX);
        return Plugin_Handled;
    }
    ...
    for (new i = 0; i < strlen(text); i++)
    {
        ...
        {
            PrintToChat(client,
                        "%s You can only use \x03%d\x04 letters every \x03%02d:%02d",
                        MSG_PREFIX, max, GetMaxInterval(client) / 60, GetMaxInterval(client) % 60);
            break;
        }
    ...
    PrintToChat(client,
                "%s You have \x03%d\x04 letters left",
                MSG_PREFIX, max - temp_letters);

    PrintToChat(client,
                "%s You will be allowed to use \x03%d\x04 more in \x03%02d:%02d",
                MSG_PREFIX, this_time, GetMaxInterval(client) / 60, GetMaxInterval(client) % 60);

    LogMessage("%L laser letters - %s", client, text);
    CreateTimer(float(GetMaxInterval(client)), SubtractLetters, hData, TIMER_FLAG_NO_MAPCHANGE);
...
GetMaxInterval(client)
{
    new bits = GetUserFlagBits(client);
    new Float:retval = float(iMaxInterval);
...
stock DrawLetter(const String:letter[], Float:origin[3], Float:angles[3])
{
    ...
    do
    {
        ...
        TE_SetupBeamPoints(tempOrigin, tempEnd, g_iSpriteBeam,
...
public Action:SubtractLetters(Handle:timer, any:hData)
{
    ...
    {
        PrintToChat(client,
                    "%s You can now use \x03%d\x04 more letters (\x03%d\x04 total)",
                    MSG_PREFIX, subtract, GetMaxPerTime(client) - (letters - subtract));
    }
