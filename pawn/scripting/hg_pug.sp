...
new Handle:hCheckPlayersReady = INVALID_HANDLE;
new Handle:hMaxPlayers;
new g_iMaxPlayers = 10;
...
new String:TeamPlayers[TEAM_COUNT][MAXPLAYERS+1][24]; // Steam ID's. Cached before map change.
new String:TeamPlayersMoney[TEAM_COUNT][MAXPLAYERS+1][5]; // for leave money storage
...
{
    for(new x=0;x<g_iMaxPlayers/2;x++)
    {
    ...
    for(new x=1;x<=MAXPLAYERS;x++)
    {
        if(ValidClient(x) && !IsFakeClient(x))
        {
            if(ReadyUpState())
            {
                ...
                new String:tmptextReady[256];
                new String:tmptextNotReady[256];
                ...
                for(new y=1;y<=MAXPLAYERS;y++)
                {
                    if(ValidClient(y) && !IsSourceTV(y))
                    {
                        if(bReady[y] && GetClientTeam(y) > 1) // only show players on a team
                        {
                            ...
                            Format(tmptextReady, 256, "%s%s\n", tmptextReady, plNameTrun);
                            //Format(optComma, 32, ", ");
                        }
                    }
                }
                ...
                // do notready players
                new plNotReady = 0;
                new plMaxShow = 0; // 193 char avail, 8 = 176 used each plname = 22
                for(new y=1;y<=MAXPLAYERS;y++)
                {
                    if(ValidClient(y) && !IsSourceTV(y))
                    {
                        if(!bReady[y] && GetClientTeam(y) > 1 && plMaxShow < 8) // only show players on a team, max of 8 for not ready
                        {
                            plNotReady++;
                            plMaxShow++;
                            new String:plName[32];
                            ...
                            Format(tmptextNotReady, 256, "%s%s\n", tmptextNotReady, plNameTrun);
                        }
                    }
                }
                
                if(g_iMaxPlayers > 10){
                    Format(tmptext, 256, "READY (%d):\nYou are %s\n", plReady, (bReady[x])?"Ready!":"NotReady\nType .ready to play");
                    Format(tmptext, 256, "%s\nNOT READY (%d):\n%s", tmptext, plNotReady, tmptextNotReady);
                }
                else
                {
                    Format(tmptext, 256, "READY:\n%s", tmptextReady);
                    Format(tmptext, 256, "%s\nNOT READY:\n%s", tmptext, tmptextNotReady);
                }
                ...
                if(CaptainMode && gMatchState==MS_Before_First_Half) // if they switched maps show this
                {
					if(TournamentMode)
					{
						PrintHintTextToAll("Waiting on Players to join their team...\n.ready up when you're on a team!");
					} else {
						PrintHintTextToAll("Waiting on Captains to pick players...\n.ready up when you're on a team!");
					}
                }
            }
        }
...
public Action:NextMapDelayed(Handle:timer)
{
    new String:curmap[32];
    GetCurrentMap(curmap, 32);
    if(!StrEqual(curmap, MatchMap))
    {
        ForceChangeLevel(MatchMap, "Setting up next match");
    }
}
...
AddSteamToTeam(const String:steamID[], teamNum)
{
    ...
    if(TeamCount<g_iMaxPlayers/2)
    {
        //PrintToServer("DEBUG: TeamCount: %i g_iMaxPlayers: %i", TeamCount, g_iMaxPlayers/2); // removeme
        for(new x=0;x<g_iMaxPlayers/2;x++)
        {
...
EnforceMatchCvars(bool:ot = false)
{
    ...
    else if(Ruleset==Rules_CGS || Ruleset==Rules_2V2)
    {
...
Rules2V2()
{
    PrintToChatAll("%s Ruleset will be: (5 Round Halves, $8000)", MSG_PREFIX);
    Ruleset = Rules_2V2;
    TeamVote();
}
...
StartMatchSetup()
{
    ...
    if(Mode2V2){
	    Rules2V2(); //StartRulesVote(); skip rules vote
    } else {
...
ReadyUp(client) {
    ...
    if(!bStillMore)
    {
        if(PlCount == g_iMaxPlayers)
        {
            OnAllReady();
        }
        else
        {
            new NeedPl = g_iMaxPlayers - PlCount;
...
public Action:SayHook(client,args,bool:team)
{
    ...
    {
        PrintToChat(client, "%s Commands: .ready, .notready, .help, .dmg, .autodmg, .mute, .chat", MSG_PREFIX);
        bHookMessage = true;
    }
    else
    {
...
public Action:HookJoinTeam(client, const String:command[], argc) 
{
    ...
    else if(firstParamNumber == CS_TEAM_T)
    {
        ...
        if(ActivePlayers > g_iMaxPlayers || GetTeamClientCount(CS_TEAM_T) >= g_iMaxPlayers/2)
        {
    ...
    else if(firstParamNumber == CS_TEAM_CT)
    {
        // if teams are full dont allow them to join, i.e. spectators trying to join
        new ActivePlayers = TeamSizeActive(CSTeamToCSL(CS_TEAM_T)) + TeamSizeActive(CSTeamToCSL(CS_TEAM_CT));
        if(ActivePlayers > g_iMaxPlayers || GetTeamClientCount(CS_TEAM_CT) >= g_iMaxPlayers/2)
        {
    ...
    else // Autojoin, our own version.
    {
        // if teams are full dont allow them to join, i.e. spectators trying to join
        new ActivePlayers = TeamSizeActive(CSTeamToCSL(CS_TEAM_T)) + TeamSizeActive(CSTeamToCSL(CS_TEAM_CT));
        if(ActivePlayers > g_iMaxPlayers)
        {
...
ClearMatch()
{
    ...
    // Are we IN TOURNEY mode? map rotation
    if(TournamentMode)
    {
        new String:curmap[32];
        ...
        if(!StrEqual(curmap, nextmap))
        {
            Format(MatchMap, 32, nextmap);
            PrintToChatAll("%s Map will change in 15 seconds for next match. %s", MSG_PREFIX, MatchMap);
            CreateTimer(15.0, NextMapDelayed);
		}
	}
}
...
public OnPluginStart()
{
    ...
    hMaxPlayers = CreateConVar("sv_maxplayers", MAX_PLAYERS_DEFAULT, "Match size for readyup.", FCVAR_PLUGIN|FCVAR_SPONLY, true, 2.0, true, 64.0);
    new Float:fMaxPlayers = GetConVarFloat(hMaxPlayers);
    g_iMaxPlayers = ((RoundToCeil(fMaxPlayers) % 2)? RoundToCeil(fMaxPlayers)+1 : RoundToCeil(fMaxPlayers) );
	...
	hMode2V2 = CreateConVar("sv_pug_2v2", "0", "Enable/Disable Tournament Mode.", FCVAR_PLUGIN|FCVAR_SPONLY);
    Mode2V2 = GetConVarInt(hMode2V2) ? true : false;
    ...
    HookConVarChange(hMaxPlayers, OnConVarChanged);
...
public OnConVarChanged(Handle:CVar, const String:oldv[], const String:newv[])
{
    if (CVar == hMaxPlayers)
    {
        new Float:fMaxPlayers = GetConVarFloat(hMaxPlayers);
        g_iMaxPlayers = ((RoundToCeil(fMaxPlayers) % 2)? RoundToCeil(fMaxPlayers)+1 : RoundToCeil(fMaxPlayers) );
        //PrintToServer("DEBUG: g_iMaxPlayers: %i", g_iMaxPlayers);
        if(hCheckPlayersReady!=INVALID_HANDLE)
        {
            CloseHandle(hCheckPlayersReady);
        }
        hCheckPlayersReady = CreateTimer(22.0, CheckPlayersOver, _, TIMER_REPEAT|TIMER_FLAG_NO_MAPCHANGE);
    }
}

public Action:CheckPlayersOver(Handle:timer)
{
    if (ReadyUpState())
    {
        //PrintToServer("DEBUG: ReadyUpState");
        new bool:bStillMore = false;
        new PlCount = 0;
        for(new a=1;a<=MAXPLAYERS;a++)
        {
            //PrintToServer("DEBUG: a: %i PlCount: %i", a, PlCount);
            if(ValidClient(a) && !IsSourceTV(a) && GetClientTeam(a) > 1)
            {
                //PrintToServer("DEBUG: valid a: %i PlCount: %i", a, PlCount);
                PlCount++;
                if(!bReady[a])
                {
                    bStillMore = true;
        ...
        if(!bStillMore)
        {
            ...
            else if(PlCount > g_iMaxPlayers)
            {
                new NeedPl = PlCount - g_iMaxPlayers;
                PrintToChatAll("%s The Admin has set this match for %d vs %d... %d player%s need%s to leave.", MSG_PREFIX, g_iMaxPlayers/2, g_iMaxPlayers/2, NeedPl, (NeedPl>1)?"s":"", (NeedPl>1)?"":"s");
            }
            else
            {
                new NeedPl = g_iMaxPlayers - PlCount;
...
TeamOfSteamId(const String:steamID[])
{
    ...
    for(new x=0;x<TEAM_COUNT;x++)
    {
        for(new y=0;y<g_iMaxPlayers/2;y++)
        {
