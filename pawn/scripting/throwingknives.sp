...
new String:g_sKnifeMdl[PLATFORM_MAX_PATH];
...
public APLRes:AskPluginLoad2(Handle:myself, bool:late, String:error[], err_max)
{
    ...
    CreateNative("IsEntityThrowingKnife", NativeIsEntityThrowingKnife);
    ...
    if (StrEqual(game, "cstrike"))
    {
        ...
        Format(g_sKnifeMdl, sizeof(g_sKnifeMdl), "models/weapons/w_knife_ct.mdl");
    }

    else
    {
        g_iGame = GAMETYPE_CSGO;
        Format(g_sKnifeMdl, sizeof(g_sKnifeMdl), "models/weapons/w_knife.mdl");
    }

    RegPluginLibrary("throwingknives");
...
public OnPluginStart() {
    ...
    g_CVarEnableDev = CreateConVar("sm_throwingknives_dev", "1", "Enable/disable dev. mode.", _, true, 0.0, true, 1.0);
    g_CVarVelocity = CreateConVar("sm_throwingknives_velocity", "5", "Velocity (speed) adjustment.", _, true, 1.0, true, 10.0);
    g_CVarKnives = CreateConVar("sm_throwingknives_count", "0", "Amount of knives players spawn with.", _, true, 0.0, true, 100.0);
    ...
    g_CVarSteal = CreateConVar("sm_throwingknives_steal", "0", "If enabled, knife kills get the victims remaining knives. 0 = Disabled | 1 = Melee kills only | 2 = All knife kills", _, true, 0.0, true, 2.0);
...
public OnMapStart() {
    ...
    g_iKnifeMI = PrecacheModel(g_sKnifeMdl);
...
public EventWeaponFire(Handle:event,const String:name[],bool:dontBroadcast) { // only fires for primary attack
    ...
    if (GetConVarBool(g_CVarEnable)) {
        ...
        if (!IsFakeClient(client) && (StrContains(sWeapon, "knife") > -1 || StrContains(sWeapon, "bayonet") > -1) && (g_iKnives[client] > 0))
            ThrowKnife(client);
    }
}
...
ThrowKnife(client) {
    ...
    // Prevent people from getting stuck in each other
    CreateTimer(6.66, Timer_ReduntantKillKnife, entity);

    if (entity > -1)
    {
        SDKHook(entity, SDKHook_Spawn, FB_OnEntitySpawned);
    }

    if ((entity != -1) && DispatchSpawn(entity)) {
        SetEntityModel(entity, g_sKnifeMdl);
    ...
    {
        new ind = FindValueInArray(g_hLethalArray, entity);
        if (ind > -1)
            RemoveFromArray(g_hLethalArray, ind);
    }
}

public FB_OnEntitySpawned(entity)
{
    if (IsValidEntity(entity))
    {
        CreateTimer(0.0, Timer_RemoveThinkTick, entity, TIMER_FLAG_NO_MAPCHANGE);
        SDKHook(entity, SDKHook_ShouldCollide, FB_OnShouldCollide);
...
public Action:Timer_ReduntantKillKnife(Handle:timer, any:entity)
{
    new index = FindValueInArray(g_hLethalArray, entity);
...
{
    if (GetConVarBool(g_CVarEnable) && StrContains(sample, "weapons/flashbang/grenade_hit1.wav", false) > -1) {
        new index = FindValueInArray(g_hLethalArray, entity);
        if (index != -1) {
            volume = 0.2;
...
stock DoHurt(victim, attacker, entity)
{
    ...
    if (IsClientInGame(client)) {
        switch (g_iDisplay) {
            ...
            case 2: { // Key Hint
                ...
                {
                    PbSetInt(hKHT, "msg_dst", 1);
                    PbSetString(hKHT, "hints", sBuffer);
                }
...
public NativeIsEntityThrowingKnife(Handle:plugin, numParams)
{
    new entity = GetNativeCell(1);
    return FindValueInArray(g_hLethalArray, entity) > -1;
}
