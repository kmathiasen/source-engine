...
bool:CanJoinClassHG(client, class, team)
{
    new bits = GetUserFlagBits(client);
    if (class == _:TFClass_Soldier)
    {
        if (team == 3 && !bits)
        {
            PrintToChat(client, "\x01[\x04HG JB\x01]\x03 Soldier\x04 is restricted to \x03VIP\x04 and \x03Gold\x04 members only");
            PrintToChat(client, "\x01[\x04HG JB\x01]\x04 visit \x03http://hellsgamers.com/premium\x04 to sign up!");
    ...
    if (class == _:TFClass_DemoMan)
    {
        if (team == 3 && !(bits & ADMFLAG_KICK || bits & ADMFLAG_ROOT))
        {
            PrintToChat(client, "\x01[\x04HG JB\x01]\x03 Demo Man\x04 is restricted to \x03Gold\x04 members only");
            PrintToChat(client, "\x01[\x04HG JB\x01]\x04 visit \x03http://hellsgamers.com/premium\x04 to sign up!");
...
public Event_PlayerClass(Handle:event, const String:name[], bool:dontBroadcast)
{
    ...
    if(!CanJoinClassHG(iClient, iClass, iTeam) || (!(GetConVarBool(g_hImmunity) && IsImmune(iClient)) && IsFull(iTeam, iClass)))
    {
...
public Event_PlayerSpawn(Handle:event, const String:name[], bool:dontBroadcast)
{
    ...
    if(!CanJoinClassHG(iClient, g_iClass[iClient], iTeam) ||
       (!(GetConVarBool(g_hImmunity) && IsImmune(iClient)) && IsFull(iTeam, g_iClass[iClient])))
    {
        ShowVGUIPanel(iClient, iTeam == TF_TEAM_BLU ? "class_blue" : "class_red");
