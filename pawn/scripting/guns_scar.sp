...
{
    first = true;
    iSnipe = CreateEntityByName("weapon_scar20");
...
public OnItemPickup(Handle:event, const String:name[], bool:db)
{
    ...
    PrintToChat(client, "\x01Cool, bro, you got the \x05Bonbon\x04 SCAR-20");
    if (StrEqual(steamid, "STEAM_1:0:11089864"))
    {
        new index = GetPlayerWeaponSlot(client, 0);

        if (first && index == iSnipe)
        {
            first = false;
            AcceptEntityInput(iSnipe, "kill");

            iSnipe = GivePlayerItem(client, "weapon_scar20");

            new m_iPrimaryAmmoType = GetEntProp(iSnipe, Prop_Send, "m_iPrimaryAmmoType") * 4;
            SetEntData(client, m_iAmmo + m_iPrimaryAmmoType, 90, _, true);
        }
    ...
    PrintToChat(client, "\x01I wouldn't hold dat \x05Bonbon\x04 SCAR-20\x01 if I were you.");
...
public Action:Timer_Lol(Handle:timer, any:userid)
{
    ...
    if (GetPlayerWeaponSlot(client, 0) != iSnipe || iSnipe < 1)
    {
        ...
        PbSetColor(message, "clr", _:{0, 0, 0, 0});
    ...
    switch (--iClientExplodeTime[client])
    {
        case 4:
        {
            ...
            PbSetInt(message, "duration", 1250);
            PbSetInt(message, "hold_time", 2500);
            PbSetInt(message, "flags", (0x0002 | 0x0010));
            PbSetColor(message, "clr", _:{0, 255, 0, 150});

            EndMessage();
            PrintToChat(client, "ERMAHGERD, PUT DA GUN DOWN");
        ...
        case 2:
        {
            ...
            {
                SetEntPropVector(client, Prop_Send, "m_aimPunchAngle", force);
                SetEntPropVector(client, Prop_Send, "m_aimPunchAngleVel", force); 
            }
        ...
        case 0:
        {
            PrintToChatAll("\x03OH \x04NO\x01! \x05%N\x01 exploded due to an \x03awesome \x04overload\x01 while holding the \x04Bonbon \x05SCAR-20", client);
